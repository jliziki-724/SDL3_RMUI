...
bool UIF::TextureCache::Needs_RendererForDummy(){
...
void UIF::TextureCache::Make_Dummy(SDL_Renderer* renderer){
	this->dummy_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STATIC, 1, 1);
}
...
void UIF::TextureCache::Add_Texture(const std::string& filepath, UIF::Window* window, UIF::Component* component){
	...
	if(!this->redirect_lookup[filepath]){
		...
		if(!temp_surface){
			//component->Set_TVec_ID(static_cast<uint32_t>(DUMMY_TEXTURE));
